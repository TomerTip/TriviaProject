...
public:
	...
	bool update_game_status(int id);
	vector<string> get_personal_status(string username);
	int rc;
	sqlite3* db;
	char* zErrMsg = 0;
	bool is_exist;
	...
	static string _correct;
	static string _false;
	static string _avg_time;
	static string _num_of_games;
...
private:
	...
	static int callback_correct(void*, int, char**, char**);
	static int callback_false(void*, int, char**, char**);
	static int callback_time(void*, int, char**, char**);
	static int callback_games(void*, int, char**, char**);
