...
void TriviaServer::handleRecievedMessages()
{
	...
	while (true)
	{
		...
		case ANS:
			...
			cout << ">> User's Sock = " << rm->getSock() << ": ANS" << endl;
			break;
		case LEAVE_GAME:
			handleLeaveGame(rm);
		...
		case BEST_SCORES:
			...
			cout << ">> User's Sock = " << rm->getSock() << ": BEST_SCORES" << endl;
			break;
		case MY_STATUS:
			handleGetPersonalStatus(rm);
			cout << ">> User's Sock = " << rm->getSock() << ": MY_STATUS" << endl;
			break;
		case END:
			safeDeleteUser(rm);
...
User* TriviaServer::handleSignin(RecievedMessage* m)
{
	...
	{
		if (this->_db.is_user_and_pass_match(username, password))
		{
...
{
	string first = this->_db.get_best_scores()[0];
	string second = this->_db.get_best_scores()[1];
	string third = this->_db.get_best_scores()[2];
...
{
	string message;
	string username = m->getUser()->get_user_name();

	string gamesAmount = this->_db.get_personal_status(username)[0];
	string correct = this->_db.get_personal_status(username)[1];
	string wrong = this->_db.get_personal_status(username)[2];
	string avgTime = this->_db.get_personal_status(username)[3];

	if (gamesAmount == "0" || correct == "0" || wrong == "0") //if empty:
	{
		m->getUser()->send(RES_MY_STATUS_EMPTY);
	}
	else
	{
		gamesAmount = Helper::getPaddedNumber(stoi(gamesAmount), 4);
		correct = Helper::getPaddedNumber(stoi(correct), 6);
		wrong = Helper::getPaddedNumber(stoi(wrong), 6);
		avgTime = Helper::getPaddedNumber(stoi(avgTime), 4);

		message.append(RES_MY_STATUS);
		message.append(gamesAmount);
