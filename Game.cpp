...
void Game::send_question_to_all_users(){
	...
	for (unsigned int i = 0; i < this->_users.size(); i++)
	{
		...
		{
			cout << "Error: sending question to user." << endl;
		}
...
void Game::handle_finish_game()
{
	...
	for (map<string, int>::iterator it = this->_results.begin(); it != this->_results.end(); it++)
	{
		...
		message.append(Helper::getPaddedNumber(it->second, 2));
	}
...
bool Game::handle_next_turn()
{
	if (!this->_users.empty())
	{
		...
		{
			if (this->_curret_question_index != this->_question_num - 1) //if it wasn't the last question:
			{
...
{	
	bool correct;
	string message = RES_ANS;
	string ans;
	int ansIndex = (ansNo < 5) ? ansNo-1 : ansNo; //if the user didn't answer in time, ansNo = 5 and, there is no answer.

	int correctIndex = this->_questions[this->_curret_question_index]->get_correct_ans_index();

	if (ansIndex != 5)//if the user has answered in time:
	{
		ans = this->_questions[this->_curret_question_index]->get_answers()[ansIndex];
	...
	if (correct) //if the user answered the correct answer:
	{
		...
		{
			this->_db.add_ans_to_player(this->_id, user->get_user_name(), this->_curret_question_index, ans, correct, time);
			this->_results[user->get_user_name()]++; //adds a point to the user;
			message.append("1");
			user->send(message); //correct answer.
		}
		else
		{
			this->_db.add_ans_to_player(this->_id, user->get_user_name(), this->_curret_question_index, "", correct, time); //user didn't answer in time.
			message.append("0");
			user->send(message);
		}
	}
	else
	{
		this->_db.add_ans_to_player(this->_id, user->get_user_name(), this->_curret_question_index, ans, correct, time);
		message.append("0");
		user->send(message); //wrong answer.
	}
