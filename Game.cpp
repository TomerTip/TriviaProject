...
bool Game::handle_answer_from_user(User* user, int ansNo, int time)
{
	...
	if (ansNo == this->_questions[this->_curret_question_index]->get_correct_ans_index()) //if the user answered the correct answer:
	{
		this->_results[user->get_user_name()]++; //adds a point to the user;

		if (ansNo != 5) //if the user answered  in time:
		{
			//this->_db.addAnswerToPlayer(ans);
			user->send(RES_ANS + '1'); //correct answer.
		}
		else
		{
			//this->_db.addAnswerToPlayer(empty);
			//user->send(RES_ANS + '?');
		}
	}
	else
